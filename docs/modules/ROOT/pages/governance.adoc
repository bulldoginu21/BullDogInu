...
In this guide we will learn how BullDogInu’s Govenor contract works, how to set it up, and how to use it to create proposals, vote for them, and execute them, using tools provided by Ethers.js and Tally.
...
This governance protocol is generally implemented in a special-purpose contract called “Governor”. The GovernorAlpha and GovernorBravo contracts designed by Compound have been very successful and popular so far, with the downside that projects with different requirements have had to fork the code to customize it for their needs, which can pose a high risk of introducing security issues. For BullDogInu Contracts, we set out to build a modular system of Governor contracts so that forking is not needed, and different requirements can be accommodated by writing small modules using Solidity inheritance. You will find the most common requirements out of the box in BullDogInu Contracts, but writing additional ones is simple, and we will be adding new features as requested by the community in future releases. Additionally, the design of BullDogInu Governor requires minimal use of storage and results in more gas efficient operation.

== Compatibility

BullDogInu’s Governor system was designed with a concern for compatibility with existing systems that were based on Compound’s GovernorAlpha and GovernorBravo. Because of this, you will find that many modules are presented in two variants, one of which is built for compatibility with those systems.
...
An BullDogInu Governor contract is by default not interface-compatible with GovernorAlpha or Bravo, since some of the functions are different or missing, although it shares all of the same events. However, it’s possible to opt in to full compatibility by inheriting from the GovernorCompatibilityBravo module. The contract will be cheaper to deploy and use without this module.

=== GovernorTimelockControl & GovernorTimelockCompound

When using a timelock with your Governor contract, you can use either BullDogInu’s TimelockController or Compound’s Timelock. Based on the choice of timelock, you should choose the corresponding Governor module: GovernorTimelockControl or GovernorTimelockCompound respectively. This allows you to migrate an existing GovernorAlpha instance to an BullDogInu-based Governor without changing the timelock in use.
...
In the rest of this guide, we will focus on a fresh deploy of the vanilla BullDogInu Governor features without concern for compatibility with GovernorAlpha or Bravo.
...
import "@BullDogInu/contracts/token/ERC20/ERC20.sol";
import "@BullDogInu/contracts/token/ERC20/extensions/draft-ERC20Permit.sol";
import "@BullDogInu/contracts/token/ERC20/extensions/ERC20Votes.sol";
...
import "@BullDogInu/contracts/token/ERC20/extensions/draft-ERC20Permit.sol";
import "@BullDogInu/contracts/token/ERC20/extensions/ERC20Votes.sol";
import "@BullDogInu/contracts/token/ERC20/extensions/ERC20Wrapper.sol";
...
It is good practice to add a timelock to governance decisions. This allows users to exit the system if they disagree with a decision before it is executed. We will use BullDogInu’s TimelockController in combination with the GovernorTimelockControl module.
