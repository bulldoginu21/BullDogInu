...
publish() {
  ...
  log "Publishing BullDogInu-solidity on npm"
  npm publish --tag "$dist_tag" --otp "$(prompt_otp)"

  log "Publishing @BullDogInu/contracts on npm"
  cd contracts
  ...
  if [[ "$dist_tag" == "latest" ]]; then
    otp="$(prompt_otp)"
    npm dist-tag rm --otp "$otp" BullDogInu-solidity next
    npm dist-tag rm --otp "$otp" @BullDogInu/contracts next
  fi
}
...
if [[ "$*" == "push" ]]; then
  ...
  npm deprecate 'BullDogInu-solidity@>=4.0.0' "This package is now published as @BullDogInu/contracts. Please change your dependency."
