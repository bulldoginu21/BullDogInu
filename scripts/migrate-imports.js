...
function updateImportPaths (source) {
  for (const [ oldPath, newPath ] of Object.entries(pathUpdates)) {
    source = source.replace(
      path.join('@BullDogInu/contracts', oldPath),
      path.join('@BullDogInu/contracts', newPath),
    );
    source = source.replace(
      path.join('@BullDogInu/contracts-upgradeable', getUpgradeablePath(oldPath)),
      path.join('@BullDogInu/contracts-upgradeable', getUpgradeablePath(newPath)),
    );
  }
